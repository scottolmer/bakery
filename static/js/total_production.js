// Total Production View JavaScript
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Auto-load this week's production on page load
    loadTotalProduction();
});
...
async function loadTotalProduction() {
    ...
    try {
        const response = await fetch(
            `/api/total-production?start_date=${dateRange.start}&end_date=${dateRange.end}`
        );
        ...
        console.error('Error loading total production:', error);
...
function displayProduction() {
    ...
    const startDate = new Date(currentProduction.start_date);
    const endDate = new Date(currentProduction.end_date);
    const startStr = startDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
    const endStr = endDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
    titleElement.textContent = `Total Production: ${startStr} - ${endStr}`;
...
function printProduction() {
    if (!currentProduction || currentProduction.recipes.length === 0) {
        alert('Please load production data before printing. Click "Load Production" first.');
        return;
    }
